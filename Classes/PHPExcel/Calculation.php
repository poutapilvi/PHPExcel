...
class PHPExcel_Calculation
{
    ...
    public function parseFormula($formula)
    {
        ...
        if (empty($formula) || (substr($formula, 0, 1) != '=')) {
            return array();
        }
...
//echo 'Element with value '.$opCharacter.' is an Operator', PHP_EOL;
                while ($stack->count() > 0 &&
					...
					isset(self::$operatorAssociativity[$opCharacter]) &&
...
//echo 'Element is a Closing bracket', PHP_EOL;
                ...
                $d = $stack->last(2);
                if (!empty($d['value']) && preg_match('/^'.self::CALCULATION_REGEXP_FUNCTION.'$/i', $d['value'], $matches)) {    //    Did this parenthesis just close a function?
